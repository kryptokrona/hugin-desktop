<script>
	...
	onMount(async () => {
		...
		if (savedMode === 'color' && customColor) {
			...
			initialTheme = themes[savedThemeName]
				? themes[savedThemeName][savedMode]
				: themes['neutral']['dark'];
		}
		...
		if (initialTheme) {
			...
			root.style.setProperty(
				...
				initialTheme.primaryForeground || '#fff'
			);
	...
	window.api.receive('privateMsg', (data) => {
		...
		const activeConversation =
			$user.activeChat?.conversation || $user.activeChat?.chat || $user.activeChat?.address;
		const inchat =
			conversation === activeConversation && $misc.focus && $page.url.pathname === '/messages';

		if ($misc.focus && $sounds.on && !inchat) new_message_sound.play();
		if (!$misc.focus) window.api.send('notify-dm', data);

		if (conversation === activeConversation && !$misc.focus && $page.url.pathname === '/messages')
			return;
	...
	window.api.receive('user-joined-voice-channel', (data) => {
		...
		let contact = $user.contacts.find(
			(a) => (a.chat || a.conversation || a.address) == data.address
		);
		if (!contact) return;
	...
	const setDownloadStatus = (data) => {
		...
		let file = $remoteFiles.find(
			(a) => a.hash === data.hash || (a.fileName === data.fileName && a.chat === data.chat)
		);
		if (!file) file = { ...data };
		file.progress = 0;
