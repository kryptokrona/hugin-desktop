<script>
	...
	const addNewRoom = async (e) => {
		...
		if (!$rooms.roomArray.some((r) => r.key === room.key)) {
			$rooms.roomArray = [
				...$rooms.roomArray,
				{ key: room.key, name: room.name, nick: '', msg: '', message: '' }
			];
		}
		printRoom(room, true);
	};
		...
		<div class="right_side" out:fade|global={{ duration: 100 }}>
			...
			{#if replyTrue}
				<div class="reply_to_exit" class:reply_to={replyTrue} onclick={() => replyExit()}>
					{reply_exit_icon}
					{t('replyTo', { nick: $rooms.replyTo.nick }) || `Reply to ${$rooms.replyTo.nick}`}
				</div>
			{/if}
			{#if usersTyping > 0}
				<div class="typing">
