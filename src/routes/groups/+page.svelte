<script>
    ...
    onMount(async () => {
        ...
        if ($groups.groupArray.length) {
            $groups.thisGroup =
                $groups.groupArray[$groups.groupArray.length - 1]
        }
    ...
    async function checkReactions() {
        ...
        filterBoards = await $groupMessages.filter(
            (m) =>
                m.message.length > 0 &&
                !(m.reply.length === 64 && containsOnlyEmojis(m.message))
        )
        //Only reactions
        ...
        <div class="outer" id="chat_window">
            {#each fixedGroups as message (message.hash)}
                <GroupMessage
                    ...
                    on:replyTo={(e) =>
                        replyToMessage(message.hash, message.name)}
                    {message}
                    reply={message.reply}
                    msg={message.message}
                    myMsg={message.sent}
