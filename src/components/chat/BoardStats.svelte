<script>
    ...
    function filterActiveBoards(arr) {
        let uniq = {}
        active_boards = arr.filter(
            (obj) => !uniq[obj.board] && (uniq[obj.board] = true)
        )
    }
...
<div class="wrapper">
    <div class="top" on:click={toggleBoardSettings}>
        ...
        {#if removeBoard}
            <div in:fade class="remove">
                <p style="color: var(--warn-color)" on:click={removeThisBoard}>
                    Remove
                </p>
            </div>
        {/if}
    ...
    <div class="list-wrapper">
        {#each active_boards as board}
            <ActiveBoard
                {board}
                on:click={() => dispatch('printBoard', board.board)}
            />
        {/each}
    </div>
</div>
