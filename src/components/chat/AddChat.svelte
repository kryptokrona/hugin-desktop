...
<div on:click|self in:fade="{{duration: 70}}" out:fade="{{duration: 100}}" class="backdrop">
    ...
    <div in:fly="{{y: 20}}" out:fade class="field">
        {#if step === 1}
            ...
            {/if}
            <input out:fade placeholder="Enter Hugin address" type="text" spellcheck="false" autocomplete="false"
                   bind:value={text}>
    ...
    {#if pubkey && step === 2 }
        ...
        <div in:fade class="field">
            ...
            {/if}
            <input in:fade placeholder="Enter a nickname" type="text" spellcheck="false" autocomplete="false"
                   bind:value={nickname}>
            <div style="width: 100px; margin-left: 10px">
                <FillButton disabled={!enableButton} enabled={step == 2 && nickname.length} on:click={handleAdd}
                            text="Add"/>
            </div>
        </div>
    {/if}
