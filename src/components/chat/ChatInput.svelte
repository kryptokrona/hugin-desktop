<script>
    ...
    $: {
        if ($user.activeChat) {
            off_chain = $webRTC.call.some(
                (a) => a.chat == $user.activeChat.chat && a.connected
            )
            console.log('offchain', off_chain)
        }
    }
...
<div
    class="wrapper"
    class:rtc
    class:border-bottom={$page.url.pathname === '/boards'}
...
>
    ...
    <button disabled={!enableSend} class:enableSend on:click={sendMsg}>
        <SendIcon />
    </button>
</div>
