...
ipcMain.handle('get-hugin-node', async () => {
	if (Object.keys(Hugin.huginNode).length === 0) {
		Hugin.huginNode = store.get('huginNode') ?? { address: '', pub: true };
	}
	return Hugin.huginNode;
});
...
class Account {
	...
	async load() {
		...
		const newTopics = allTopics.filter((t) => !this.syncImages.includes(t));
		if (newTopics.length > 0) {
			this.syncImages = [...this.syncImages, ...newTopics];
