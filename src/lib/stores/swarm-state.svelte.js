...
function isMap(v) {
    return v instanceof Map || (v != null && typeof v.has === 'function' && typeof v.delete === 'function' && typeof v.values === 'function')
}
...
class Peers {
    ...
    isDmOnline(chatAddress) {
        return this.swarms.some(s =>
            s.chat === chatAddress && s.connections?.some(c => c.address === chatAddress)
        )
    }
    ...
    onSwarmConnected(payload) {
        ...
        if (!incoming) return
        const idx = this.swarms.findIndex(s =>
            (incoming.topic && s.topic === incoming.topic) ||
            (incoming.key && s.key === incoming.key)
        )
        if (idx >= 0) {
            ...
            this.swarms[idx] = {
                ...current,
                ...incoming,
                voice_channel: isMap(current.voice_channel) ? current.voice_channel : incoming.voice_channel,
